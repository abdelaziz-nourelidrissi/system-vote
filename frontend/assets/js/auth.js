...
function showSignupForm() {
  ...
  document.getElementById('signup-form').addEventListener('submit', function (e) {
    ...
    if (isValid) {
      ...
      fetch("backend/api/signup.php", { //Fetch, n'oublie pas de changer après hébergement
        method: "POST",
        ...
        .then(data => {
          if (data === "VALID_FOR_NEXT_STEP") {
            ...
            requestAnimationFrame(() => {
              fetch("backend/api/send-code.php", { //Fetch, n'oublie pas de changer après hébergement
                method: "POST",
                headers: { "Content-Type": "application/x-www-form-urlencoded" },
...
function showLoginForm() {
  ...
  document.getElementById('login-form').addEventListener('submit', function(e) {
    ...
    fetch("backend/api/login.php", { //Fetch, n'oublie pas de changer après hébergement
      method: "POST",
      headers: { "Content-Type": "application/x-www-form-urlencoded" },
      body: `email=${encodeURIComponent(email)}&password=${encodeURIComponent(password)}&fingerprint=${encodeURIComponent(fingerprint)}`
    })
    .then(response => response.text())
    .then(data => {
...
function showVerificationStep(nom, prenom, email, password, fingerprint) {
  ...
  function verifyCode(code, nom, prenom, email, password, fingerprint) {
    ...
    fetch("backend/api/verify_code.php", {//Fetch, n'oublie pas de changer après hébergement
      method: "POST",
      headers: { "Content-Type": "application/x-www-form-urlencoded" },
