...
async function getTimerData() {
    try {
        ...
        console.error('Erreur lors de la récupération des données du timer:', error);
...
async function getCurrentTime() {
    try {
        ...
        console.log("Heure récupérée depuis l'API:", currentTime);
    } catch (error) {
        console.error('Erreur API (backend):', error);
        currentTime = new Date(); // Utilise l'heure locale si l'API échoue
        console.log("Utilisation de l'heure locale:", currentTime);
    }
}
